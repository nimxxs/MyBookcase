...
const getLibrary = async () => {
  const url = new URL(
    `https://www.nl.go.kr/seoji/SearchApi.do?cert_key=${API_KEY}&result_style=json&page_no=1&page_size=300&start_publish_date=20220509&end_publish_date=20220509`
  );
  ...
  const newsHTML = newsList
    ...
    .map((news, index) => {
      ...
      const truncatedTitle =
        news.TITLE.length > 10 ? news.TITLE.slice(0, 20) + "..." : news.TITLE;
