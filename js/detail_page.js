...
let targetBookDetail = async () => {
  ...
  //ISBN = "9788941241256"
  url = new URL(
    `https://www.nl.go.kr/seoji/SearchApi.do?cert_key=${API_KEY}&result_style=json&page_no=1&page_size=10&isbn=${ISBN}`
  );
  ...
  url2 = new URL(
    `https://www.nl.go.kr/NL/search/openApi/search.do?apiType=json&key=${API_KEY}&detailSearch=true&isbnOp=isbn&isbnCode=${ISBN}`
  );
...
function renderHeart(ISBN) {
  ...
  if (existingEntry) {
    if (conditionValue === false) {
      ...
      document.querySelector(
        ".wishbutton"
      ).innerHTML = `<img width="20px" src="/../images/heart.fill 1.svg"/>`;
    }
  } else {
...
function publishDate(bookInfo) {
  if (bookInfo.REAL_PUBLISH_DATE !== "") {
    return bookInfo.REAL_PUBLISH_DATE;
  }
  if (bookInfo.REAL_PUBLISH_DATE == "") {
    if (bookInfo.PUBLISH_PREDATE !== "") {
      return bookInfo.PUBLISH_PREDATE;
    } else {
      return "";
    }
  }
